...
class ForecastModel(nn.Module):
    def __init__(self, backbone, num_features, seq_len, process_method='RevIN', **kwargs):
        ...
        elif process_method.lower() == 'transfer':
            self.processor = Transfer(num_features=num_features, seq_len=seq_len, **kwargs)
        else:
...
class MoBN(nn.Module):
    def __init__(self, num_features: int, eps=1e-5, affine=True, num_bn=2, **kwargs):
        """
        :param num_features: the number of features or channels
        :param eps: a value added for numerical stability
        :param affine: if True, RevIN has learnable affine parameters
        """
        super(MoBN, self).__init__()
        self.num_features = num_features
        self.num_bn = num_bn
